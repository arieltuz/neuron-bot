...
Usa InlineKeyboard en el flujo /convertir para evitar conflictos con entry_points.
"""
...
def _guardar_presupuesto(numero, datos): db.guardar_presupuesto(numero, datos)
def _buscar_presupuesto(numero): return db.buscar_presupuesto(numero)
...
def fmt_pesos(v): return f"$ {v:,.2f}".replace(",","X").replace(".",",").replace("X",".")

# Teclados
def teclado_menu():
    return ReplyKeyboardMarkup(
        [["📄 Presupuesto", "🧾 Comprobante X"], ["🧾 Factura A/B", "❓ Ayuda"]],
...
def inline_conv_tipo():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🧾 Comprobante X", callback_data="conv_comprobante")],
        [InlineKeyboardButton("🅰️ Factura A", callback_data="conv_factura_a"),
         InlineKeyboardButton("🅱️ Factura B", callback_data="conv_factura_b")],
        [InlineKeyboardButton("❌ Cancelar", callback_data="conv_cancelar")],
    ])

def inline_confirmar_factura():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Emitir Factura", callback_data="fac_confirmar")],
        [InlineKeyboardButton("❌ Cancelar", callback_data="fac_cancelar")],
    ])
...
async def get_texto(update: Update):
    ...
    if msg.voice or msg.audio:
        ...
        await msg.reply_text("🎙️ Transcribiendo...")
        try:
            ...
            await msg.reply_text("⚠️ No pude entender el audio. Escribí el texto.")
            return None
...
async def ayuda(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/convertir `<número>` — Convertir presupuesto _(ej: /convertir 5)_\n"
...
async def menu_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    tl = texto.lower()
    if any(p in tl for p in ["presupuesto","cotiz","precio","📄"]):
        return await iniciar_presupuesto(update, ctx)
    elif "factura" in tl:
        return await iniciar_factura(update, ctx)
    elif any(p in tl for p in ["comprobante","recibo","compro"]):
...
async def pres_mas_items(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if any(p in texto.lower() for p in ["sí","si","dale","otro","más","mas","yes"]):
...
async def pres_confirmar(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_document(
        ...
        filename=f"Presupuesto_{numero}_NeuronComputacion.pdf",
        caption=(f"✅ *Presupuesto N° {numero}* generado.\n\n"
                 f"💡 Si el cliente lo aprueba, convertilo con:\n`/convertir {numero}`"),
        parse_mode="Markdown", reply_markup=teclado_menu())
    ctx.user_data.clear(); return ConversationHandler.END
...
async def comp_confirmar(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    pdf_bytes = generar_comprobante_x_pdf(
        numero=numero,
        cliente_nombre=ctx.user_data.get("cliente_nombre","-"),
        cliente_cuit=ctx.user_data.get("cliente_cuit","-"),
        cliente_tel=ctx.user_data.get("cliente_tel","-"),
        items=ctx.user_data["items"],
        notas=ctx.user_data.get("notas",""))
    await update.message.reply_document(
        ...
        caption=f"✅ *Comprobante X N° {numero}* generado.",
        parse_mode="Markdown", reply_markup=teclado_menu())
    ctx.user_data.clear(); return ConversationHandler.END
...
async def fac_tipo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if "cancel" in texto.lower(): return await cancelar(update, ctx)
...
async def fac_cliente_doc(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data["fac_tipo"] == "A" and doc_tipo != 80:
        await update.message.reply_text("⚠️ Para *Factura A* el cliente debe tener CUIT (11 dígitos). Reingresá:", parse_mode="Markdown")
        return FAC_CLIENTE_DOC
    ctx.user_data["cliente_doc_tipo"] = doc_tipo
    ctx.user_data["cliente_doc_nro"] = doc_nro
    ctx.user_data["cliente_cond_iva"] = (
        "IVA Responsable Inscripto" if ctx.user_data["fac_tipo"] == "A"
        else "Monotributista / Exento" if doc_tipo == 80
        else "Consumidor Final")
    await update.message.reply_text(
        f"✅ Cliente: {ctx.user_data['cliente_nombre']} ({ctx.user_data['cliente_cond_iva']})\n\n"
        "📦 Ítems.\n\n✏️ *Descripción* del ítem:", parse_mode="Markdown")
...
async def fac_item_qty(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if qty is None:
        ...
        return FAC_ITEM_QTY
    ctx.user_data["_item_qty"] = qty
    lbl = "SIN IVA" if ctx.user_data["fac_tipo"] == "A" else "FINAL (con IVA)"
    await update.message.reply_text(f"✏️ *Precio unitario {lbl}* (sin $):", parse_mode="Markdown")
...
async def fac_mas_items(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if any(p in texto.lower() for p in ["sí","si","dale","otro","más","mas","yes"]):
        await update.message.reply_text("✏️ Descripción del siguiente ítem:", reply_markup=ReplyKeyboardRemove())
        return FAC_ITEM_DESC
    return await _mostrar_resumen_factura(update.message, ctx)

async def _mostrar_resumen_factura(msg, ctx):
    items = ctx.user_data["items"]
...
async def fac_confirmar(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await _emitir_factura(update.message, ctx)
    return ConversationHandler.END

async def fac_confirmar_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if query.data == "fac_cancelar":
        await query.edit_message_text("❌ Factura cancelada.")
        ctx.user_data.clear()
        await query.message.reply_text("¿Qué querés hacer?", reply_markup=teclado_menu())
        return ConversationHandler.END
    await query.edit_message_text("⏳ Emitiendo factura en ARCA...")
    await _emitir_factura(query.message, ctx)
    return ConversationHandler.END

async def _emitir_factura(msg, ctx):
    try:
        ...
        qr_b64 = generar_qr_afip(
            CUIT, resultado["fecha"], resultado["punto_venta"], tipo_cmp,
            resultado["numero"], resultado["total"],
            ctx.user_data["cliente_doc_tipo"], ctx.user_data["cliente_doc_nro"],
            resultado["cae"])
        pdf_bytes = generar_factura_pdf(
            tipo=ctx.user_data["fac_tipo"], numero=resultado["numero"],
            punto_venta=resultado["punto_venta"], cae=resultado["cae"],
            vencimiento_cae=resultado["vencimiento_cae"], fecha=resultado["fecha"],
            cliente_nombre=ctx.user_data["cliente_nombre"],
            cliente_doc_tipo=ctx.user_data["cliente_doc_tipo"],
            cliente_doc_nro=ctx.user_data["cliente_doc_nro"],
            cliente_cond_iva=ctx.user_data["cliente_cond_iva"],
        ...
        await msg.reply_text(f"❌ Error al emitir:\n`{str(e)[:400]}`",
                             parse_mode="Markdown", reply_markup=teclado_menu())
    ctx.user_data.clear()

# CONVERTIR PRESUPUESTO — usa InlineKeyboard para evitar conflictos con entry_points
async def convertir_presupuesto(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    args = ctx.args if ctx.args else []
    if args:
        return await _mostrar_para_convertir(update, ctx, args[0].strip())
    await update.message.reply_text(
        "🔄 *Convertir Presupuesto*\n\n✏️ Escribí el *número* del presupuesto:\n_(ej: 5 o 0005)_",
        parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
    return CONV_NUMERO
...
async def conv_numero(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    return await _mostrar_para_convertir(update, ctx, texto.strip())

async def _mostrar_para_convertir(update: Update, ctx: ContextTypes.DEFAULT_TYPE, numero: str):
    datos = _buscar_presupuesto(numero)
    if not datos:
        await update.message.reply_text(
            f"⚠️ No encontré el presupuesto N° {numero.zfill(4)}.\nEscribí otro número o /cancelar:",
            reply_markup=ReplyKeyboardRemove())
        return CONV_NUMERO
    ctx.user_data["conv_datos"] = datos
    ctx.user_data["conv_numero"] = numero.zfill(4)
    items = datos["items"]
    total = sum(it["qty"]*it["precio"] for it in items)
    detalle = "\n".join(f"  • {it['qty']}x {it['desc']} — {fmt_pesos(it['precio'])}" for it in items)
    await update.message.reply_text(
        f"📄 *Presupuesto N° {numero.zfill(4)} encontrado:*\n\n"
        f"👤 {datos.get('cliente_nombre','-')}\n"
        f"🪪 {datos.get('cliente_dni','-')}\n\n"
        f"*Ítems:*\n{detalle}\n\n*TOTAL: {fmt_pesos(total)}*\n\n"
        "¿A qué lo querés convertir?",
        parse_mode="Markdown",
        reply_markup=inline_conv_tipo())   # ← InlineKeyboard, no puede disparar entry_points
    return CONV_TIPO

async def conv_tipo_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Callback de InlineKeyboard — nunca puede ser interceptado por entry_points."""
    query = update.callback_query
    await query.answer()
    data = query.data
    datos = ctx.user_data.get("conv_datos", {})
    numero_presupuesto = ctx.user_data.get("conv_numero", "?")

    if data == "conv_cancelar":
        await query.edit_message_text("❌ Conversión cancelada.")
        ctx.user_data.clear()
        await query.message.reply_text("¿Qué querés hacer?", reply_markup=teclado_menu())
        return ConversationHandler.END

    if data == "conv_comprobante":
        numero = next_number("comprobante")
        await query.edit_message_text("⏳ Generando Comprobante X...")
        pdf_bytes = generar_comprobante_x_pdf(
            numero=numero,
        ...
        await query.message.reply_document(
            document=io.BytesIO(pdf_bytes),
            filename=f"ComprobanteX_{numero}_NeuronComputacion.pdf",
            caption=f"✅ *Comprobante X N° {numero}* generado desde Presupuesto N° {numero_presupuesto}.",
            parse_mode="Markdown", reply_markup=teclado_menu())
        ctx.user_data.clear()
        return ConversationHandler.END

    # Factura A o B
    tipo_fac = "A" if data == "conv_factura_a" else "B"
    from arca_handler import detectar_tipo_doc
    ...
    ctx.user_data["cliente_cond_iva"] = (
        "IVA Responsable Inscripto" if tipo_fac == "A"
        else "Monotributista / Exento" if doc_tipo == 80
        else "Consumidor Final")
    ...
    await query.edit_message_text(
        f"🧾 *Resumen Factura {tipo_fac}* (desde Presupuesto N° {numero_presupuesto})\n\n"
        f"👤 {ctx.user_data['cliente_nombre']}\n"
        f"🪪 {doc_nro or '-'}\n"
        f"📋 {ctx.user_data['cliente_cond_iva']}\n\n"
        f"*Ítems:*\n{detalle}\n\n"
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(filters.Regex(r"(?i)(📄 presupuesto|^presupuesto$|cotiz)"), iniciar_presupuesto),
            MessageHandler(filters.Regex(r"^🧾 Comprobante X$"), iniciar_comprobante),
            MessageHandler(filters.Regex(r"^🧾 Factura A/B$"), iniciar_factura),
        ],
        states={
            ...
            FAC_CONFIRMAR: [
                MessageHandler(voz_o_texto & ~filters.COMMAND, fac_confirmar),
                CallbackQueryHandler(fac_confirmar_callback, pattern="^(fac_confirmar|fac_cancelar)$"),
            ],
            CONV_NUMERO: [MessageHandler(voz_o_texto & ~filters.COMMAND, conv_numero)],
            CONV_TIPO:   [CallbackQueryHandler(conv_tipo_callback, pattern="^conv_")],
        },
    ...
    # Handler global para callbacks de factura (por si viene desde conv_tipo_callback)
    app.add_handler(CallbackQueryHandler(fac_confirmar_callback, pattern="^(fac_confirmar|fac_cancelar)$"))
